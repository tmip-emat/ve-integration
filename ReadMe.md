...
> [!WARNING]
> The models contained in this example are drawn from those used by ODOT
> and are meant to be illustrative of the kinds of analysis that can be
> done using these tools. However, these models as published in this repository
> have certain proprietary data removed and replaced with synthetic datasets.
> The synthetic datasets mimic the formatting and structure of the proprietary
> data, allowing the example to run, but do not represent actual data. Thus, 
> no policy conclusions should be drawn from the results of the example models.
...
5. **Scenario-Inputs/OTP** - This directory contains scenario input files in subdirectories for each experiment parameter defined by the scope in *odot-otp-scope.yml*.
...
1. Build the 'VisionEval' runtime environment: 
...
3. Install and Run VE-State Full OTP model
	...
	-	After successful installation of all the modules from ODOT_VE_Extras navigate to "C:\VisionEval-Dev\built\visioneval\4.3.1\runtime" and double click     VisionEval-dev.Rproj. This will open RStudio, set up the VisionEval environment and give access to ODOT models.
...
4. Run the scenarios
	...
	-	Enter the command `jupyter notebook` and press *Enter*. This will open a jupyter notebook in a browser and list all the files contained in the **C:\ve-integration** directory.
	...
	-	Click on *ODOT-TMIP-METAMODEL.ipynb*. This will open the jupyter notebook.
	-	Check the values of following parameters in the **Cell Block 2**:
		...
		-	*database_name*: A character value that tells the name of the database. If one doesn't exist then the notebook will create one. Note that if the notebook is creating the database then it cannot load results and the run_experiments should be set to True.
	...
	- 	Run the remaining cells of the jupyter notebook to run the scenarios and visualize the results. 
