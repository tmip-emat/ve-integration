...
CalculateHouseholdDvmtSpecifications <- list(
  ...
  Set = visioneval::items(
    visioneval::item(
      ...
      ISELEMENTOF = "",
      SIZE = 0,
      DESCRIPTION = items("Average daily vehicle miles traveled by the household in autos or light trucks",
                          "Average daily vehicle miles traveled by the household in autos or light trucks (unadjusted output of the AADVMT model)")
    ),
    visioneval::item(
      NAME = items(
        "UrbanHhDvmt",
...
CalculateHouseholdDvmt <- function(L) {
  ...
  # Set CommuteDistanceAdj to 1 if the variable is not available
  if(is.null(L$Year$Household$CommuteDistanceAdj)){
    L$Year$Household$CommuteDistanceAdj <- rep(1, length(L$Year$Household$HhId))
  }
  # Set FractionWorkersAtHome to 0 if the variable is not available
  if(is.null(L$Year$Household$FractionWorkersAtHome)){
    L$Year$Household$FractionWorkersAtHome <- rep(0, length(L$Year$Household$HhId))
  }
