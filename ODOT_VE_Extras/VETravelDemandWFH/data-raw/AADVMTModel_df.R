...
Fmlas_df <- tribble(
  ...
  "metro",    function(x, pow=0.43) x^(1/pow), ~lm(I(AADVMT^0.43) ~ Drivers + Workers+LogIncomeK+Age0to14+Age65Plus+log1p(VehPerDriver) + LifeCycle+
                                                                      CENSUS_R+FwyLaneMiPC+TranRevMiPC:D4c+D1B+D2A_WRKEMP+D3bpo4 +
                                                                      offset(0.03198915 * CommuteDistanceAdj - 0.1979329 * FractionWorkersAtHome),
                                                                     data= ., weights=.$hhwgt, na.action=na.exclude),
  "non_metro",function(x, pow=0.43) x^(1/pow), ~lm(I(AADVMT^0.43) ~ Drivers+HhSize+Workers+CENSUS_R+LogIncomeK+Age0to14+Age65Plus+log1p(VehPerDriver)+LifeCycle+
                                                                       D1B*D2A_EPHHM +
                                                                         offset(0.03471072 * CommuteDistanceAdj - 0.2147726 * FractionWorkersAtHome),,
                                                                     data= ., weights=.$hhwgt, na.action=na.exclude)
)
...
#' don't use trim model function with offsets!
